...
checkpoint_path='/data/pch/work1/TT-master/outlog/UCF-slowfast_resnet50-KD-Base-10%/'
train_state_path='/data/pch/work1/TT-master/outlog/UCF-slowfast_resnet50-KD-Base-10%/'
...
if __name__ == '__main__':
    ...
    teacher.load_state_dict(torch.load('/data/pch/work1/test/checkpoint/slowfast_resnet101.ckpt'))
        ...
        if i%5==1:
            with tqdm.tqdm(choose_loader , total=math.floor(len(choose_loader)), ncols=0) as pbar:
                ...
                for n_iter, (input_data, target, index ) in enumerate(pbar):
                    ...
                    if i%1==0:
                        mask_ratio= min(1-pow((1 - 1.0 * (i-1-resume) /cfg.CONFIG.TRAIN.EPOCH_NUM),0.9),0.5)
                        if mask_ratio<0.5:
                    ...
                    for j,idx in enumerate(sample_data.index):
                        ...
                        if correct==48 : 
                            if mask_ratio<0.5:
                                featurelist[idx]=featurelist[idx]+0.01*(1-mask_ratio)*sample_data.feature[j]
                ...
                with torch.no_grad():
                    ...
                    for j,idx in enumerate(sample_data.index):
                        if i==1:
                            ...
                            hisloss[idx]=0.1*hisloss[idx]+0.9*mask_ratio*sample.loss
            kernel_matrix = compute_kernel_matrix(sample_list)
            indices,lindice = dpp_sample(kernel_matrix, sample_list, math.floor(0.1*(len(choose_loader))))
